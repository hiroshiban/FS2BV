...
if(strcmpi(ext,'.gz'))
  ...
  elseif strcmp(computer,'PCWIN') || strcmp(computer,'PCWIN64')
    %error=unix(sprintf('gunzip -c %s > %s', niftifile, new_niftifile))
    [T,error]=evalc('unix(sprintf(''gunzip -c %s > %s'', niftifile, new_niftifile))');
    if error
      unix(sprintf('gzip -d -c %s > %s', niftifile, new_niftifile));
    end
  else
