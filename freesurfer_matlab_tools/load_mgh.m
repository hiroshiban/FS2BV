...
% 171224 Hiroshi Ban, modified to be available on Windows
...
if (strcmpi(fname((strlen(fname)-3):strlen(fname)), '.MGZ') | ...
  ...
  new_fname = sprintf('tmp%d.mgh', gzipped);
  if(strcmp(computer,'MAC') || strcmp(computer,'MACI') || ismac)
    unix(sprintf('gunzip -c %s > %s', fname, new_fname));
  elseif strcmp(computer,'PCWIN') || strcmp(computer,'PCWIN64')
    %error=unix(sprintf('gunzip -c %s > %s', fname, new_fname));
    [T,error]=evalc('unix(sprintf(''gunzip -c %s > %s'', fname, new_fname))'); %#ok
    if error
      unix(sprintf('gzip -d -c %s > %s', fname, new_fname)) ;
    end
  else
