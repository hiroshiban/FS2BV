function poi=GenerateSphericalPOIfromTALcoords(SRF_file,center_tal_XYZ,radius_mm,poinames,output_coordinate,poicolors,save_name)

% Generates spherical POIs in TAL space.
% function GenerateSphericalPOIfromTALcoords(SRF_file,center_tal_XYZ,radius_mm,poinames,:output_coordinate,:poicolors,:save_name)
% (: is optional)
%
% This function generates spheric POI(s) as the center='center_tal_XYZ' with radius='radius_mm'
%
% [example]
% >> center_tal_XYZ=[-35.3,-80.8,5.0;31.7,-84.7,2.5];
% >> poinames={'KO_left_tal','KO_right_tal'};
% >> save_name='KO_talairach.poi';
% >> SRF_file='/HB/3d/zk09_034_LH_inflated.srf';
% >> poi=GenerateSphericalPOIfromTALcoords(SRF_file,center_tal_XYZ,5,poinames,'TAL',[],save_name);
%
% [input]
...
% Last Update: "2020-03-30 16:41:50 ban"
...
if nargin<6 || isempty(poicolors)
  poicolors=cell(size(center_tal_XYZ,1),1);
...
if size(center_tal_XYZ,1)~=length(poinames)
  error('the numbers of center_tal_XYZ and poinames mismatch. check input variable');
end
...
if size(center_tal_XYZ,1)==1
  fprintf('Generating spheric POI (radius: %02dmm) from Talairach coordinates\n',radius_mm);
else
  fprintf('Generating spheric POIs (radius: %02dmm) from Talairach coordinates\n',radius_mm);
end
...
center_poi_XYZ=[128-center_tal_XYZ(:,1),128-center_tal_XYZ(:,2),128-center_tal_XYZ(:,3)];
...
for ii=1:1:size(center_poi_XYZ,1)
  ...
  fprintf('processing (TAL:% 6.1f,% 6.1f,% 6.1f) as % 14s...',...
          center_tal_XYZ(ii,1),center_tal_XYZ(ii,2),center_tal_XYZ(ii,3),poinames{ii});
