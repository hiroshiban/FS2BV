function [voi,voi_voxels,nii]=ConvertNiftiRoi2BVvoi_ProbThres(NII_dir,thres,input_coordinate,output_coordinate,voi_color,prefix_nii,save_flg)

% Reads a ROI defined as a probability map in a NIFTI file and converts it to a BrainVoyager VOI file by thresholiding the NIFTI image values (probabilities).
% function [voi,voi_voxels,nii]=ConvertNiftiRoi2BVvoi_ProbThres(NII_dir,:thres,:input_coordinate,:output_coordinate,:voi_color,:prefix_nii,:save_flg)
% (: is optional)
%
...
% thres      : (optional) threshold in defining VOI, [min, max]
%              the values are required if you are going to convert a ROI stored in
%              a NIFTI file as a probability (0.0-1.0, or 0.0-100.0). the voxels in which
%              thres(1) < voxel_value <= thres(2) will be defined as VOI voxels.
%              [0.0,1.0] by default.
...
%                    'TAL' by default
% voi_color  : (optional) VOI color, [R(0-255),G,B]. [127,255,255] by default.
...
%
%
% Created    : "2017-08-28 16:13:48 ban"
% Last Update: "2021-06-16 10:02:38 ban"

% check the input variables
if nargin<1 || isempty(NII_dir), help(mfilename()); return; end
if nargin<2 || isempty(thres), thres=[0.0,1.0]; end
...
if nargin<5 || isempty(voi_color), voi_color=[127,255,255]; end
if nargin<6 || isempty(prefix_nii), prefix_nii=''; end
if nargin<7 || isempty(save_flg), save_flg=1; end

if numel(thres)==1, error('thres should be [min,max]. check input variables'); end
...
for ii=1:1:length(niifiles)
  ...
  % get the voxel index which exceeds the 'thres'
  voi_name{ii}=niifname;%nii.fileprefix;
  voi_idx=find( thres(1)<nii.img & nii.img<=thres(2) );
  voi_voxels{ii}=zeros(numel(voi_idx),3);
  [voi_voxels{ii}(:,1),voi_voxels{ii}(:,2),voi_voxels{ii}(:,3)]=ind2sub(size(nii.img),voi_idx);
  ...
  if isempty(nii.hdr.hist.rot_orient) || numel(nii.hdr.hist.rot_orient)~=3
    voi_voxels{ii}(:,1)=abs(nii.hdr.hist.srow_x(1))*(voi_voxels{ii}(:,1)-(nii.hdr.hist.originator(1)-1));
    voi_voxels{ii}(:,2)=abs(nii.hdr.hist.srow_y(2))*(voi_voxels{ii}(:,2)-(nii.hdr.hist.originator(2)-1));
    voi_voxels{ii}(:,3)=abs(nii.hdr.hist.srow_z(3))*(voi_voxels{ii}(:,3)-(nii.hdr.hist.originator(3)-1));
  else
    voi_voxels{ii}(:,1)=abs(nii.hdr.hist.srow_x(nii.hdr.hist.rot_orient(1)))*(voi_voxels{ii}(:,1)-(nii.hdr.hist.originator(1)-1));
    voi_voxels{ii}(:,2)=abs(nii.hdr.hist.srow_y(nii.hdr.hist.rot_orient(2)))*(voi_voxels{ii}(:,2)-(nii.hdr.hist.originator(2)-1));
    voi_voxels{ii}(:,3)=abs(nii.hdr.hist.srow_z(nii.hdr.hist.rot_orient(3)))*(voi_voxels{ii}(:,3)-(nii.hdr.hist.originator(3)-1));
  end

  % convert data coordinate, MNI <--> TAL
  if strcmpi(input_coordinate,'MNI') && strcmpi(output_coordinate,'TAL')
    voi_voxels{ii}=mni2tal(voi_voxels{ii});
    voi_voxels{ii}=unique(voi_voxels{ii},'rows');
  elseif strcmpi(input_coordinate,'TAL') && strcmpi(output_coordinate,'MNI')
    voi_voxels{ii}=tal2mni(voi_voxels{ii});
    voi_voxels{ii}=unique(voi_voxels{ii},'rows');
  ...
  voi{ii}.NrOfVOIs=1;
  voi{ii}.VOI(1).Name=voi_name{ii};
  voi{ii}.VOI(1).Color=voi_color;
  voi{ii}.VOI(1).NrOfVoxels=size(voi_voxels{ii},1);
  voi{ii}.VOI(1).Voxels=voi_voxels{ii};
