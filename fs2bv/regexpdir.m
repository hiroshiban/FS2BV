...
for h = find([dirtree.isdir]);
  if ~any(strcmp({'.','..'}, dirtree(h).name));
    if regexpi(strrep(fullfile(rootdir, dirtree(h).name, filesep), basedir, ''), expstr);
      dirlist = [dirlist; fullfile(rootdir, dirtree(h).name, filesep)]; %#ok
    end
    if recursive;
      dirlist = [dirlist; regexpdir(fullfile(rootdir, dirtree(h).name, filesep), expstr, recursive)]; %#ok
    end
  end
end
for i = find(~[dirtree.isdir]);
  if regexpi(strrep(fullfile(rootdir, dirtree(i).name), basedir, ''), expstr);
    dirlist = [dirlist; fullfile(rootdir, dirtree(i).name)]; %#ok
  end
end
