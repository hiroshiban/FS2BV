function [voi,voi_voxels,nii]=ConvertAALroi2BVvoi(XML_dir,input_coordinate,output_coordinate,prefix_xml,save_flg)

% Reads ROI XML file(s) defined by AAL (automated anatomical labeling) atlas and converts the corresponding NII file to BrainVoyager VOI file(s).
% function [voi,voi_voxels,nii]=ConvertAALroi2BVvoi(XML_dir,:input_coordinate,:output_coordinate,:prefix_xml,:save_flg)
% (: is optional)
%
...
%              is called is the origin. e.g. NII_dir='../../ROI_nii'
...
%              e.g. prefix_xml='Cerebellum_MNIflirt', empty by default.
% save_flg   : (optional) whether saving the generated BrainVoayer VOI. [0|1]
%              1 by default. when the target ROI nii_file is 'ABC.nii.gz', then the VOI
...
%
% [note: 1]
% This function can handle non-1mm ROI files (e.g. 2mm or 0.5mm space), but I recommend to use
% ROI defined in 1mm space that is the default of BrainVoayer VOI space. If non-1mm ROIs are used,
% each of the generated VOIs will only sparsely cover the target regions. Please be careful.
%
% [note: 2]
% if you want to convert a specific ROI as BrainVoayer VOI files, it may be better to use
% ConvertSPMroi2BVvoi function with some ROI extraction procedures below.
%
% [note: 3]
% AAL.nii and AAL2.nii are provided as ROIs defined in 2mm cubic volume space.
% if you want to convert them to 1mm cubic space, that is compatible with BrainVoayger
% default anatomical volume space, please run
% >> reslice_nii('AAL.nii','AAL_1mm.nii',1,'','',2);
% before using this function. here, reslice_nii is in BVQX_hbtools/nifti_tools.
%
% >> % read and extract target ROIs
% >> nii=load_niigz(fullfile(pwd,'FSL_atlases/talairach-daemon-atlas/Talairach/Talairach-labels-1mm.nii.gz'));
% >> nii.img(nii.img ~= 622 & nii.img ~= 623)=0;
% >>
% >> % change index
% >> nii.img(nii.img==622)=1; % left pulvinar index
% >> nii.img(nii.img==623)=1; % right pulvinar index
% >>
% >> % tmporally save the extracted ROIs
% >> save_niigz(nii,fullfile(pwd,'Talairach-Pulvinar_LGN_labels-1mm.nii.gz'));
%
% >> % convert the extracted ROIs to VOIs
% >> ConvertSPMroi2BVvoi('..',[1,2],'MNI','TAL');
%
...
% ref: http://neuro.imm.dtu.dk/wiki/Automated_Anatomical_Labeling
%
% [dependency]
% 1. BVQXtools v0.8d
...
if nargin<2 || isempty(input_coordinate), input_coordinate='MNI'; end
if nargin<3 || isempty(output_coordinate), output_coordinate='TAL'; end
...
% processing
voi_name=cell(length(xmlfiles),1);
voi_voxels=cell(length(xmlfiles),1);
voi=cell(length(xmlfiles),1);
for ii=1:1:length(xmlfiles)
  ...
  % the codes to get NII file prefix below are messy, just generated based on current FSL data conventions
  ...
  % get the ROI data
  ROIdata=xml.getElementsByTagName('data').item(0).getElementsByTagName('label');
  nROIs=ROIdata.getLength();
  ROIs=cell(ROIdata.getLength(),1);
  for rr=1:1:nROIs
    tgt_roi=ROIdata.item(rr-1); % as item starts from 0
    ROIs{rr}.name=char(tgt_roi.getElementsByTagName('name').item(0).getFirstChild.getData());
    ROIs{rr}.index=str2num(char(tgt_roi.getElementsByTagName('index').item(0).getFirstChild.getData())); %#ok
    fprintf('  [%03d] % 20s\n',ROIs{rr}.index,ROIs{rr}.name);
  end
  fprintf('\n');

  % omit if the background (label: *.*.*.*.*) is included
  if strcmp(unique(ROIs{1}.name),'*.')
    ROIs=ROIs(2:end);
    nROIs=nROIs-1;
  end

  % set VOI color(s)
  voi_color=rainbow(nROIs);
  ...
  if isempty(niifiles)
    ...
    if isempty(niifiles)
      ...
      if isempty(niifiles)
        ...
        if isempty(niifiles)
          warning('can not find any NII file in the target directory. check the input variables. skipping...');
          continue
        end
      end
    end
  ...
  % converting from FSL ROIs to BV VOIs
  voi_name{ii}=cell(length(niifiles),1);
  voi_voxels{ii}=cell(length(niifiles),nROIs);
  voi{ii}=cell(length(niifiles),1);
  for mm=1:1:length(niifiles)
    if strcmp(niifiles{mm}(end-3:end),'.voi'), continue; end % skip if some VOI file is unexpectedly selected above.

    % load the nii file
    [niipath,niifname,niiext]=fileparts(niifiles{mm});
    fprintf('converting: %s%s --> %s.voi...',niifname,niiext,niifname);
    ...
    % here, I assumu that nii.hdr.hist.rot_orient is generally [1,2,3] or [1,3,2]
    voi{ii}{mm}=BVQXfile('new:voi');
    if strcmpi(output_coordinate,'MNI'), voi{ii}{mm}.ReferenceSpace='MNI'; end % actually MNI has not been supported in BrainVoyager v2.8
    voi{ii}{mm}.NrOfVOIs=nROIs;
    for rr=1:1:nROIs
      if max(nii.img(:))==2*nROIs
        ...
        voi_idx=find(nii.img==2*ROIs{rr}.index-1 | nii.img==2*ROIs{rr}.index);
      else
        % otherwise, we can set the ROI index as it is
        voi_idx=find(nii.img==ROIs{rr}.index);
      end
      voi_voxels{ii}{mm,rr}=zeros(numel(voi_idx),3);
      [voi_voxels{ii}{mm,rr}(:,1),voi_voxels{ii}{mm,rr}(:,2),voi_voxels{ii}{mm,rr}(:,3)]=ind2sub(size(nii.img),voi_idx);
      ...
      if isempty(nii.hdr.hist.rot_orient) || numel(nii.hdr.hist.rot_orient)~=3
        voi_voxels{ii}{mm,rr}(:,1)=abs(nii.hdr.hist.srow_x(1))*(voi_voxels{ii}{mm,rr}(:,1)-(nii.hdr.hist.originator(1)-1));
        voi_voxels{ii}{mm,rr}(:,2)=abs(nii.hdr.hist.srow_y(2))*(voi_voxels{ii}{mm,rr}(:,2)-(nii.hdr.hist.originator(2)-1));
        voi_voxels{ii}{mm,rr}(:,3)=abs(nii.hdr.hist.srow_z(3))*(voi_voxels{ii}{mm,rr}(:,3)-(nii.hdr.hist.originator(3)-1));
      else
        voi_voxels{ii}{mm,rr}(:,1)=abs(nii.hdr.hist.srow_x(nii.hdr.hist.rot_orient(1)))*(voi_voxels{ii}{mm,rr}(:,1)-(nii.hdr.hist.originator(1)-1));
        voi_voxels{ii}{mm,rr}(:,2)=abs(nii.hdr.hist.srow_y(nii.hdr.hist.rot_orient(2)))*(voi_voxels{ii}{mm,rr}(:,2)-(nii.hdr.hist.originator(2)-1));
        voi_voxels{ii}{mm,rr}(:,3)=abs(nii.hdr.hist.srow_z(nii.hdr.hist.rot_orient(3)))*(voi_voxels{ii}{mm,rr}(:,3)-(nii.hdr.hist.originator(3)-1));
      end

      % convert data coordinate, MNI <--> TAL
      if strcmpi(input_coordinate,'MNI') && strcmpi(output_coordinate,'TAL')
        voi_voxels{ii}{mm,rr}=mni2tal(voi_voxels{ii}{mm,rr});
        voi_voxels{ii}{mm,rr}=unique(voi_voxels{ii}{mm,rr},'rows');
      elseif strcmpi(input_coordinate,'TAL') && strcmpi(output_coordinate,'MNI')
        voi_voxels{ii}{mm,rr}=tal2mni(voi_voxels{ii}{mm,rr});
        voi_voxels{ii}{mm,rr}=unique(voi_voxels{ii}{mm,rr},'rows');
      ...
      if rr>1, voi{ii}{mm}.VOI(rr)=voi{ii}{mm}.VOI(1); end
      voi{ii}{mm}.VOI(rr).Name=ROIs{rr}.name;
      voi{ii}{mm}.VOI(rr).Color=voi_color(rr,:);
      voi{ii}{mm}.VOI(rr).NrOfVoxels=size(voi_voxels{ii}{mm,rr},1);
      voi{ii}{mm}.VOI(rr).Voxels=voi_voxels{ii}{mm,rr};
    end

    % save the generated VOI
    if save_flg, voi{ii}{mm}.SaveAs(fullfile(niipath,[niifname,'.voi'])); end
...
%     elseif ispc
%         try
%           [s,w]=dos(sprintf('gzip -cd %s > %s', fn, niifn));
%         catch
%           [s,w]=dos(sprintf('"%s%sbin%sgzip.exe " -cd "%s" > "%s"',fileparts(mfilename('fullpath')),filesep(),filesep(),fn,niifn));
%         end
%     end
%     if s~=0
%         error(w);
