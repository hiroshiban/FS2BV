function VolViewerMGZ(MGZ_file)

% Loads and displays a FreeSurfer MGZ anatomy volume file.
% function VolViewerMGZ(MGZ_file)
%
% This function loads a FreeSurfer MGZ anatomy file (e.g. T1.mgz) and displays
% it on the MATLB figure windows with GUI buttons for several functionalities.
%
% [input]
% MGZ_file : a FreeSurfer MGZ file with a full-path format
%            e.g. VMR_file='F:/Workspace/fMRI_preprocessed/3d/T1.mgz';
%                 or VMR_file='T1.mgz';
%
% [output]
% no output variable, the input MGZ_file is displayed on the MATLAB window.
%
% [dependency]
% FreeSurfer_matlab_tools distributed by FreeSurfer developer team
%
% [reference]
...
%
%
% Created    : "2018-01-30 09:20:12 ban"
% Last Update: "2018-02-22 13:01:05 ban"

% check the input variable
if nargin<1 || isempty(MGZ_file), help(mfilename()); return; end

if ~exist(MGZ_file,'file'), error('MGZ_file not found. check the input variable.'); end
...
% load the MGZ
[mgzpath,mgzfname,mgzext]=fileparts(MGZ_file); %#ok
fprintf('loading %s%s...',mgzfname,mgzext);
mgz=MRIread(fullfile(pwd,MGZ_file));
% this transformation below is required to convert the FreeSurfer coords to BV ones.
try
  vol=flip(permute(mgz.vol,[1,3,2]),2);
catch
  vol=flipdim(permute(mgz.vol,[1,3,2]),2);
end

framingcube=max(mgz.volsize);
...
figid=figure('Units','pixels','Name',sprintf('BVQX_hbtools VolViewerMGZ: %s%s',mgzfname,mgzext),...
