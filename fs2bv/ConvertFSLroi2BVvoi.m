function [voi,voi_voxels,nii]=ConvertFSLroi2BVvoi(XML_dir,input_coordinate,output_coordinate,prefix_xml,save_flg)

% Reads ROI XML file(s) defined by FSL software (NIFTI file) and converts it(them) to BrainVoyager VOI file(s).
% function [voi,voi_voxels,nii]=ConvertFSLroi2BVvoi(XML_dir,:input_coordinate,:output_coordinate,:prefix_xml,:save_flg)
% (: is optional)
%
...
%
%
% Created    : "2015-12-24 17:14:40 ban"
% Last Update: "2018-06-09 17:20:05 ban"
...
for ii=1:1:length(xmlfiles)
  ...
  for rr=1:1:nROIs
    ...
    ROIs{rr}.name=char(tgt_roi.getFirstChild.getData());
    if strcmp(unique(ROIs{1}.name),'*.')
      ...
      ROIs{rr}.index=str2num(char(tgt_roi.getAttribute('index'))); %#ok
    else
      % otherwise we need to set the index with +1 as 0 is the background.
      ROIs{rr}.index=str2num(char(tgt_roi.getAttribute('index')))+1; %#ok % as index starts from 0
    end
    ROIs{rr}.x=str2num(char(tgt_roi.getAttribute('x'))); %#ok
    ROIs{rr}.y=str2num(char(tgt_roi.getAttribute('y'))); %#ok
    ROIs{rr}.z=str2num(char(tgt_roi.getAttribute('z'))); %#ok
    fprintf('  [%03d] % 20s (x,y,z)=(%d,%d,%d)\n',ROIs{rr}.index,ROIs{rr}.name,ROIs{rr}.x,ROIs{rr}.y,ROIs{rr}.z);
  end
  fprintf('\n');
