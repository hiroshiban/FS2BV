function LimitVOIvoxelsByVMP(VOI_dir,vmp_file,mapnum,order,nvox,unique_flg,prefix_voi,overwrite_flg)
...
% order      : (optional) 'ascend' or 'descend', 'descend' by default
% nvox       : (optional) number of voxels to be survived after thresholding,
%              250 by default
...
%                if set to 0, the new data will be saved with '_limit{nvox}' prefix
%                in the same directory with the input VOIs. 0 by default.
%
% [output]
% no output variable,
...
% Last Update: "2018-09-03 11:33:08 ban"

% check input variable
if nargin<2, help(mfilename()); return; end
if nargin<3 || isempty(mapnum), mapnum=1; end
if nargin<4 || isempty(order), order='descend'; end
if nargin<5 || isempty(nvox), nvox=250; end
...
if ~strcmp(order,'ascend') && ~strcmp(order,'descend')
  error('order should be ''ascend'' or ''descend''. check input variable.');
end
...
for ii=1:1:length(voifiles)
  ...
  for vv=1:1:voi.NrOfVOIs
    ...
    if unique_flg
      ...
      [dummy,order_idx]=sort(mapvals,order); %#ok
    ...
    % the number of voxels is limit to 1:max_nvox
    cvox_idx=cvox_idx(1:min(nvox,numel(cvox_idx)));
  ...
  if overwrite_flg
    ...
    voi.SaveAs(fullfile(path,[voifile,sprintf('_limit%03d',nvox),voiext]));
  end
  fprintf('done.\n');
