function voi=GenerateSphericalVOIfromMNIcoords_withCortexMSK(SRF_dir,center_mni_XYZ,radius_mm,voinames,output_coordinate,voicolors,save_name,prefix_srf,srf_params,vmr_params,VMRdim)

% Generates spherical VOIs in MNI space, and masks them with a cortex mask.
% function GenerateSphericalVOIfromMNIcoords_withCortexMSK(SRF_dir,center_mni_XYZ,radius_mm,voinames,:output_coordinate,:voicolors,:save_name,:prefix_srf,:srf_params,:vmr_params,:VMRdim)
% (: is optional)
%
% This function generates spheric VOI(s) as the center='center_mni_XYZ' with radius='radius_mm'
...
% >> voinames={'KO_left_mni','KO_right_mni'};
% >> save_name='KO_mniairach.voi';
% >> voi=GenerateSphericalVOIfromMNIcoords_withCortexMSK(SRF_dir,center_mni_XYZ,5,...
%                                          voinames,'TAL',[],save_name,'*RECOSM');
%
% [input]
...
% center_mni_XYZ  : mniairach coordinate, num_voi x 3 matrix, [X1,Y1,Z1; X2,Y2,Z2,...]
...
% VMR_dim         : (optional) VMR volume dimensions in which the generated VOIs
...
%
%
% Created    : "2011-11-25 14:48:27 banh"
% Last Update: "2020-03-30 17:21:18 ban"
...
if nargin<6 || isempty(voicolors)
  voicolors=cell(size(center_mni_XYZ,1),1);
...
if size(center_mni_XYZ,1)~=length(voinames)
  error('the numbers of center_mni_XYZ and voinames mismatch. check input variable');
end
...
for ii=1:1:length(srffiles)
  ...
  if strfind(srfname,'LH') && strfind(srfname,output_coordinate)
    leftcounter=leftcounter+1;
    srfleft{leftcounter}=srffiles{ii};  %#ok
  elseif strfind(srfname,'RH') && strfind(srfname,output_coordinate)
...
if size(center_mni_XYZ,1)==1
  fprintf('Generating spheric VOI (radius: %02dmm)from Mniairach coordinates\n',radius_mm);
else
  fprintf('Generating spheric VOIs (radius: %02dmm) from Mniairach coordinates\n',radius_mm);
end
...
center_voi_XYZ=[VMRdim/2-scaling*center_mni_XYZ(:,1),VMRdim/2-scaling*center_mni_XYZ(:,2),VMRdim/2-scaling*center_mni_XYZ(:,3)];
...
for ii=1:1:size(center_voi_XYZ,1)
  ...
  % display message
  fprintf('processing (MNI:% 6.1f,% 6.1f,% 6.1f) as % 14s...',...
          center_mni_XYZ(ii,1),center_mni_XYZ(ii,2),center_mni_XYZ(ii,3),voinames{ii});
