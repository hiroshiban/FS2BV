function voi=GenerateSphericalVOIfromMNIcoords(center_mni_XYZ,radius_mm,voinames,output_coordinate,voicolors,save_name,VMRdim)

% Generates spherical VOIs in MNI space.
% function GenerateSphericalVOIfromMNIcoords(center_mni_XYZ,radius_mm,voinames,:output_coordinate,:voicolors,:save_name,:VMRdim)
% (: is optional)
%
...
% >> save_name='KO_mni.voi';
% >> voi=GenerateSphericalVOIfromMNIcoords(center_mni_XYZ,5,voinames,'TAL',[],save_name);
%
% [input]
...
% Last Update: "2020-03-30 16:44:09 ban"
...
if size(center_mni_XYZ,1)==1
  fprintf('Generating spheric VOI (radius: %02dmm)from MNI coordinates\n',radius_mm);
else
  fprintf('Generating spheric VOIs (radius: %02dmm) from MNI coordinates\n',radius_mm);
end
