function poi=GenerateSphericalPOIfromNATIVEcoords(SRF_file,center_XYZ,radius_mm,poinames,poicolors,save_name)

% Generates spherical POIs in NATIVE space.
% function GenerateSphericalPOIfromNATIVEcoords(SRF_file,center_XYZ,radius_mm,poinames,:poicolors,:save_name)
% (: is optional)
%
% This function generates spheric POI(s) as the center='center_XYZ' with radius='radius_mm'
%
% [example]
% >> center_XYZ=[-35.3,-80.8,5.0;31.7,-84.7,2.5];
% >> poinames={'KO_left','KO_right'};
% >> save_name='KO.poi';
% >> SRF_file='/HB/3d/zk09_034_LH_inflated.srf';
% >> poi=GenerateSphericalPOIfromNATIVEcoords(SRF_file,center_XYZ,5,poinames,[],save_name);
%
% [input]
...
% center_XYZ      : native XYZ coordinates, num_voi x 3 matrix, [X1,Y1,Z1; X2,Y2,Z2,...]
...
%
%
% Created    : "2020-03-30 17:05:24 ban"
% Last Update: "2020-03-30 17:13:46 ban"
...
if nargin<5 || isempty(poicolors)
  poicolors=cell(size(center_XYZ,1),1);
...
if size(center_XYZ,1)~=length(poinames)
  error('the numbers of center_XYZ and poinames mismatch. check input variable');
end
...
if size(center_XYZ,1)==1
  fprintf('Generating spheric POI (radius: %02dmm) in the NATIVE coordinates\n',radius_mm);
else
  fprintf('Generating spheric POIs (radius: %02dmm) in the NATIVE coordinates\n',radius_mm);
end
...
center_poi_XYZ=center_XYZ(:,[2,3,1]); % this is to omit a strange surface coordinate problem
...
for ii=1:1:size(center_poi_XYZ,1)
  ...
  fprintf('processing (TAL:% 6.1f,% 6.1f,% 6.1f) as % 14s...',...
          center_XYZ(ii,1),center_XYZ(ii,2),center_XYZ(ii,3),poinames{ii});
