function nii=ExtractFSLroiDirect(NII_dir,extracting_values,ROI_name,prefix_nii,save_flg)

% Reads ROI NII file(s) defined by FSL software, extracts the target value(s), and save it(them) as a separate NII file.
% function nii=ExtractFSLroiDirect(NII_dir,extracting_values,ROI_name,:prefix_nii,:save_flg)
% (: is optional)
%
...
%
% [input]
% NII_dir           : a target directory that contains ROI (*.nii or *.nii.gz) files
...
% extracting_values : value(s) to be extracted from the target NII file(s). a scalar or a vector
%                     e.g. extracting_values=[500,501];
% ROI_name          : output file name prefix, e.g. ofname='Pulvinar'; you do not need to include '_' and a file extension.
% prefix_nii        : (optional) file prefix to specify the target NII from multiple files.
%                     e.g. prefix_nii='Talairach_daemon', empty by default.
...
% >> %% some constants
% >> NII_dir='./FSL_atlases';
% >> extracting_values=[497,500];
% >> ROI_name='LGN';
% >> prefix_xml='Talairach';
% >> save_flg=1;
% >>
% >> %% processing
% >> ExtractFSLroiDirect(NII_dir,extracting_values,ROI_name,prefix_nii,save_flg);
%
...
if nargin<4 || isempty(prefix_nii), prefix_nii=''; end
...
for ii=1:1:length(niifiles)
  [dumny,niifname,niiext]=fileparts(niifiles{ii}); %#ok
  ...
  % extracting the target voxels from the input NII image
  outputimg=zeros(size(nii.img));
  for pp=1:1:numel(extracting_values), outputimg(nii.img==extracting_values(pp))=1; end
  nii.img=outputimg;
  fprintf('done.\n');
