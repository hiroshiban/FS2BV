function [voi,voi_voxels,nii]=ConvertNiftiRoi2BVvoi_Labels(NII_dir,labels,input_coordinate,output_coordinate,prefix_nii,save_flg)

% Reads ROI(s) defined as ID map(s) in a NIFTI file and converts it(tnem) to a BrainVoyager VOI file using the input label information.
% function [voi,voi_voxels,nii]=ConvertNiftiRoi2BVvoi_Labels(NII_dir,labels,:input_coordinate,:output_coordinate,:prefix_nii,:save_flg)
% (: is optional)
%
...
%                                   using a label lookuptable corresponding to the map ID
...
%
% [input]
% NII_dir    : directory that contains *.nii files, a relative path format.
%              the relative path should be set so that the location where
%              this function is called is the origin. e.g. NII_dir='../../ROI_nii'
% labels     : ROI label lookuptable, a cell structure.
%              labels=cell(N,2(ROI index, name))
%              or
%              labels=cell(N,3(ROI index, name, and RGB values)). for example,
%              labels={ROI_index_01,'name_of_ROI_01',1x3_RGB_matrix(0-255);
%                      ROI_index_02,'name_of_ROI_02',1x3_RGB_matrix;
%                      ROI_index_03,'name_of_ROI_03',1x3_RGB_matrix;
%                      .....};
%              if labels{n,3} is not set, the VOIs are painted with rainbow colors.
%              this lookup table is used to extract the ROI(s) from the input NIFTI file.
% input_coordinate : (optional) type of the coordinate of the input ROI, 'MNI' or 'TAL'
%                    'MNI' by default
% output_coordinate : (optional) type of the coordinate of the output ROI, 'MNI' or 'TAL'
%                    'TAL' by default
% prefix_nii : (optional) file prefix to specify the target NIFTI from multiple files.
%              e.g. prefix_nii='HB_', empty by default.
% save_flg   : (optional) whether saving the generated BrainVoayer VOI. [0|1]
%              1 by default. when the input nii_file is 'ABC.nii', then the VOI
%              will be saved as 'ABC.voi'
%
% [output]
% voi        : BrainVoyager BVQXtools VOI object
% voi_voxels : VOI voxels, [num_voxels x 3 (x,y,z)] matrix
...
%
%
% Created    : "2017-08-28 17:52:09 ban"
% Last Update: "2018-08-27 15:37:29 ban"

% check the input variables
if nargin<2, help(mfilename()); return; end
if nargin<3 || isempty(input_coordinate), input_coordinate='MNI'; end
if nargin<4 || isempty(output_coordinate), output_coordinate='TAL'; end
if nargin<5 || isempty(prefix_nii), prefix_nii=''; end
if nargin<6 || isempty(save_flg), save_flg=1; end

if ~iscell(labels) || ( size(labels,2)~=2 && size(labels,2)~=3 )
  error('labels should be labels=cell(N,2(ROI index, name)) or labels=cell(N,3(ROI index, name, and RGB values)). check the input variable.');
end

% add colors if they are not set
if size(labels,2)==2
  colors=rainbow(size(labels,1));
  for ii=1:1:size(labels,1), labels{ii,3}=colors(ii,:); end
end

if ~strcmpi(input_coordinate,'MNI') && ~strcmpi(input_coordinate,'TAL')
  error('input_coordinate should be ''MNI'' or ''TAL''. check input variables');
end
if ~strcmpi(output_coordinate,'MNI') && ~strcmpi(output_coordinate,'TAL')
  error('output_coordinate should be ''MNI'' or ''TAL''. check input variables');
end

% check the input nii directory
NII_dir=fullfile(pwd,NII_dir);
if ~exist(NII_dir,'dir'), error('can not find NII_dir. check input variables.'); end

% get NIFTI files
niifiles=GetFiles(NII_dir,'*.nii',prefix_nii);
if isempty(niifiles), error('can not find any NII file in NII_dir. check the input variables.'); end

% omit the irrelevant files
niiidx=logical(zeros(length(niifiles)));
for ii=1:1:length(niifiles)
  [dummy1,dummy2,niiext]=fileparts(niifiles{ii});
  if strcmpi(niiext,'.nii') || strcmpi(niiext,'.gz')
    niiidx(ii)=true;
  end
end
niifiles=niifiles(niiidx);

% processing
voi_name=cell(length(niifiles),1);
voi_voxels=cell(length(niifiles),1);
voi=cell(length(niifiles),1);
for ii=1:1:length(niifiles)
  if strcmp(niifiles{ii}(end-3:end),'.voi'), continue; end % skip if some VOI file is unexpectedly selected above.

  % load the nii file
  [niipath,niifname,niiext]=fileparts(niifiles{ii});
  fprintf('processing: %s%s ---> %s.voi...',niifname,niiext,niifname);
  if strcmpi(niiext,'.nii')
    nii=load_nii(niifiles{ii});
  elseif strcmpi(niiext,'.gz')
    nii=load_niigz(niifiles{ii});
  else
    error('can not load nii file. acceptable format is one of ''*.nii'' and ''*.nii.gz''. check the input file');
  end

  % check the consistency of the ROI IDs in the nii volume and the lookuptable stored in the labels variable
  roiidx=zeros(size(labels,1),1);
  for mm=1:1:size(labels,1)
    if numel(find(nii.img==labels{mm,1}))~=0, roiidx(mm)=1; end
  end
  voi_name{ii}=cell(sum(roiidx),1);
  voi_voxels{ii}=cell(sum(roiidx),1);

  % initialize the VOI file
  voi{ii}=BVQXfile('new:voi');
  if strcmpi(output_coordinate,'MNI'), voi{ii}.ReferenceSpace='MNI'; end % note: MNI has not been supported in BrainVoyager v2.8
  voi{ii}.NrOfVOIs=sum(roiidx);

  % get the voxel index based on the lookuptable stored in the labels variable
  roicounter=0;
  for mm=1:1:size(labels,1)

    if roiidx(mm)==0, continue; end
    roicounter=roicounter+1;

    voi_name{ii}{roicounter}=labels{mm,2};%nii.fileprefix;
    voi_idx=find( nii.img==labels{mm,1} );
    voi_voxels{ii}{roicounter}=zeros(numel(voi_idx),3);
    [voi_voxels{ii}{roicounter}(:,1),voi_voxels{ii}{roicounter}(:,2),voi_voxels{ii}{roicounter}(:,3)]=ind2sub(size(nii.img),voi_idx);

    % convert voxel sub-index to the standard MNI/TAL coordinates using NIFTI qoffset header information
    % here, nii.hdr.hist.rot_orient is generally [1,2,3] or [1,3,2], but I will prepare all the combinations just in case.
    if isempty(nii.hdr.hist.rot_orient) || numel(nii.hdr.hist.rot_orient)~=3
      voi_voxels{ii}{roicounter}(:,1)=abs(nii.hdr.hist.srow_x(1))*(voi_voxels{ii}{roicounter}(:,1)-(nii.hdr.hist.originator(1)-1));
      voi_voxels{ii}{roicounter}(:,2)=abs(nii.hdr.hist.srow_y(2))*(voi_voxels{ii}{roicounter}(:,2)-(nii.hdr.hist.originator(2)-1));
      voi_voxels{ii}{roicounter}(:,3)=abs(nii.hdr.hist.srow_z(3))*(voi_voxels{ii}{roicounter}(:,3)-(nii.hdr.hist.originator(3)-1));
    else
      voi_voxels{ii}{roicounter}(:,1)=abs(nii.hdr.hist.srow_x(nii.hdr.hist.rot_orient(1)))*(voi_voxels{ii}{roicounter}(:,1)-(nii.hdr.hist.originator(1)-1));
      voi_voxels{ii}{roicounter}(:,2)=abs(nii.hdr.hist.srow_y(nii.hdr.hist.rot_orient(2)))*(voi_voxels{ii}{roicounter}(:,2)-(nii.hdr.hist.originator(2)-1));
      voi_voxels{ii}{roicounter}(:,3)=abs(nii.hdr.hist.srow_z(nii.hdr.hist.rot_orient(3)))*(voi_voxels{ii}{roicounter}(:,3)-(nii.hdr.hist.originator(3)-1));
    end

    % convert data coordinate, MNI <--> TAL
    if strcmpi(input_coordinate,'MNI') && strcmpi(output_coordinate,'TAL')
      voi_voxels{ii}{roicounter}=mni2tal(voi_voxels{ii}{roicounter});
      voi_voxels{ii}{roicounter}=unique(voi_voxels{ii}{roicounter},'rows');
    elseif strcmpi(input_coordinate,'TAL') && strcmpi(output_coordinate,'MNI')
      voi_voxels{ii}{roicounter}=tal2mni(voi_voxels{ii}{roicounter});
      voi_voxels{ii}{roicounter}=unique(voi_voxels{ii}{roicounter},'rows');
    elseif ( strcmpi(input_coordinate,'MNI') && strcmpi(output_coordinate,'MNI') ) || ...
           ( strcmpi(input_coordinate,'TAL') && strcmpi(output_coordinate,'TAL') )
      % do nothing
    end

    % generate the VOI file
    if roicounter>1, voi{ii}.VOI(roicounter)=voi{ii}.VOI(1); end % initializing the structure by copying the first variable.
    voi{ii}.VOI(roicounter).Name=voi_name{ii}{roicounter};
    voi{ii}.VOI(roicounter).Color=labels{mm,3};
    voi{ii}.VOI(roicounter).NrOfVoxels=size(voi_voxels{ii}{roicounter},1);
    voi{ii}.VOI(roicounter).Voxels=voi_voxels{ii}{roicounter};

  end % for mm=1:1:size(labels,1)

  % save the generated VOI
  if save_flg, voi{ii}.SaveAs(fullfile(niipath,[niifname,'.voi'])); end
  fprintf('done.\n');

end % for ii=1:1:length(niifiles)

% clean up
if ~nargout
  for ii=1:1:length(voi)
    voi{ii}.ClearObject();
  end
  clear voi;
end
...
new_ext = 0;
[p,fname,ext]=fileparts(fileprefix); %#ok
if strcmpi('.nii',ext);
  new_ext = 1;
...
%     %     [s,w]=dos(sprintf('gzip -cd %s > %s',f,niifn));
%     %   catch
%     %     eval(sprintf('!"%s%sbin%sgzip.exe " -cd "%s" > "%s"',fileparts(mfilename('fullpath')),filesep(),filesep(),fn,niifn));
%     %   end
%     % end
%
%     if isunix
%         [s,w]=unix(sprintf('gzip -cd %s > %s', fn, niifn))
%     elseif ispc
%         try
%           [s,w]=dos(sprintf('gzip -cd %s > %s',f,niifn));
%         catch
